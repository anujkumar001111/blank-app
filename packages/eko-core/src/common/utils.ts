...
>(tools1: T[], tools2: T[]): T[] {
  const merged: T[] = [];
  const replacementMap = new Map<string, T>();

  // Capture the last occurrence for replacement lookups while keeping O(n) behavior.
  for (let i = 0; i < tools2.length; i++) {
    replacementMap.set(tools2[i].name, tools2[i]);
  }

  const seen = new Set<string>();
  for (let i = 0; i < tools1.length; i++) {
    const tool1 = tools1[i];
    const replacement = replacementMap.get(tool1.name);
    if (replacement) {
      merged.push(replacement);
      seen.add(tool1.name);
      replacementMap.delete(tool1.name);
    } else {
      merged.push(tool1);
      seen.add(tool1.name);
    }
  ...
  // Append new tools from tools2 in original order, deduped via Set to avoid O(n²) scans.
  for (let i = 0; i < tools2.length; i++) {
    const tool2 = tools2[i];
    if (!seen.has(tool2.name)) {
      merged.push(tool2);
      seen.add(tool2.name);
    }
  }
