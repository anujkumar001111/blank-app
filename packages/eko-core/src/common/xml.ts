/**
 * @fileoverview XML workflow parsing and agent tree construction.
 * 
 * Converts LLM-generated XML workflows into structured Workflow objects with
 * proper agent dependency trees, node relationships, and execution metadata.
 * Handles streaming XML (incomplete during generation) and final parsing.
 * 
 * @module common/xml
 */
...
/**
 * Parses XML workflow string into structured Workflow object.
 * 
 * WHY: Planner LLM generates XML, but Eko needs typed objects for execution.
 * This parser handles:
 * - Incomplete XML during streaming (done=false)
 * - Agent dependency resolution (dependsOn attributes)
 * - Parallel execution detection via tree analysis
 * - Node type inference (text, watch, forEach)
 * 
 * @param taskId - Unique task identifier for agent ID generation
 * @param xml - Raw XML string from planner (may be incomplete)
 * @param done - True if XML generation is complete, false during streaming
 * @param thinking - Optional reasoning text from O1-style models
 * @returns Parsed workflow or null if XML is too incomplete to parse
 * 
 * @example
 * ```typescript
 * const xml = `<root>
 *   <agents>
 *     <agent name="Browser" id="0"><task>Search</task></agent>
 *     <agent name="File" id="1" dependsOn="0"><task>Save</task></agent>
 *   </agents>
 * </root>`;
 * const workflow = parseWorkflow('task-123', xml, true);
 * // workflow.agents[1].dependsOn = ['task-123-0']
 * ```
 */
