...
export async function getPlanSystemPrompt(
  ...
  let systemPrompt = PromptTemplate.render(planSysPrompt, {
    name: config.name,
    agents: agents_prompt.trim(),
    examples: planExamplePrompt.trim(),
  }).trim();

  // Inject lessons from episodic memory if enabled
  if (context.config.episodicMemory) {
    try {
      const taskPrompt = context.chain.taskPrompt || "Current task";
      const relevantEpisodes = await context.config.episodicMemory.recallRelevant(taskPrompt, 5);

      if (relevantEpisodes.length > 0) {
        const lessonsInjection = context.config.episodicMemory.buildContextInjection(relevantEpisodes);
        systemPrompt += lessonsInjection;
      }
    } catch (memoryError) {
      Log.error("Failed to inject episodic lessons:", memoryError);
      // Don't fail planning if memory retrieval fails
    }
  }
