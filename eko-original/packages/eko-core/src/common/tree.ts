/**
 * @fileoverview Agent dependency tree builder for workflow execution planning.
 * 
 * Analyzes agent dependencies to construct execution trees that maximize
 * parallelization while respecting sequential constraints. Critical for
 * efficient workflow execution - runs independent agents concurrently.
 * 
 * @module common/tree
 */
...
/**
 * Builds hierarchical execution tree from flat agent list with dependencies.
 * 
 * WHY: Workflow XML specifies agents with `dependsOn` attributes, but executor
 * needs structured tree to know:
 * - Which agents can run in parallel (no dependencies between them)
 * - Which agents must run sequentially (one depends on another)
 * - What order to execute agents in (topological sort)
 * 
 * ALGORITHM:
 * 1. Detect and break circular dependencies (infinite loops)
 * 2. Find entry agents (no dependencies)
 * 3. Build tree level-by-level (BFS):
 *    - Single agent at level → NormalAgentNode
 *    - Multiple agents at level → ParallelAgentNode
 * 4. Mark agents with parallel flag for executor
 * 
 * @param agents - Flat list of agents with dependsOn arrays
 * @returns Root node of execution tree (normal or parallel)
 * 
 * @example
 * ```typescript
 * const agents = [
 *   { id: '1', dependsOn: [] },           // Entry agent
 *   { id: '2', dependsOn: ['1'] },        // Depends on 1
 *   { id: '3', dependsOn: ['1'] },        // Also depends on 1 (parallel with 2)
 *   { id: '4', dependsOn: ['2', '3'] }    // Depends on both 2 and 3
 * ];
 * const tree = buildAgentTree(agents);
 * // Result: 1 → [2, 3] (parallel) → 4
 * ```
 * 
 * @throws Error if no executable agents (all have unresolvable dependencies)
 */
