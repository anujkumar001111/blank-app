/**
 * Chat System Prompt Generator
 *
 * Builds dynamic system prompt with context injection for chat assistant.
 * Uses conditional templating (PromptTemplate) to include/exclude sections
 * based on available tools, browser state, and memory.
 *
 * Context injection sources:
 * - Tool availability: Only mentions enabled tools in instructions
 * - Retrieved memories: RAG search results from past conversations
 * - Browser tabs: Open tab metadata for webpage awareness
 * - Datetime: Current timestamp for temporal reasoning
 *
 * WHY dynamic prompts?
 * - Tool instructions vary per configuration (some chats lack browser access)
 * - Memory relevance changes per query (inject only related context)
 * - Tab awareness prevents "tab not found" errors (shows available tabs)
 * - Datetime enables time-sensitive queries ("what's the weather today?")
 *
 * Design choice: Template-based (not hardcoded) allows custom prompt
 * injection via global.prompts.set(GlobalPromptKey.chat_system, ...).
 */
...
/**
 * Base chat system prompt template with conditional sections
 *
 * Key features:
 * - <if toolName> conditional rendering (only show if tool enabled)
 * - Memory relevance warning (prevents forcing irrelevant memories)
 * - Multi-turn context warning (prevents topic drift interference)
 * - Browser tab awareness (shows available tabs for webpage_qa)
 *
 * Design philosophy: Instructs LLM to be context-aware (check memory
 * relevance before using it, treat each question independently unless
 * explicitly building on prior context).
 */
...
/**
 * Renders chat system prompt with context injection
 *
 * Performs template variable substitution:
 * - {{name}}: Agent name from config
 * - {{datetime}}: Current timestamp
 * - {{memory}}: RAG search results (if available)
 * - {{tabs}}: Browser tab list (if available)
 * - <if toolName>: Conditional sections per enabled tools
 *
 * Returns: Final system prompt string for LLM
 */
...
/**
 * Formats browser tab metadata for prompt injection
 *
 * Limits to 10 most recent tabs, truncates long titles/URLs to prevent
 * token bloat. Returns JSON structure for LLM readability.
 */
