/**
 * Vision-Based Browser Agent with Labeled Element Interaction
 *
 * Advanced browser automation using computer vision + DOM extraction hybrid.
 * Key innovation: Screenshot overlays bounding boxes with index labels,
 * enabling LLM to reference elements via coordinates OR indices.
 *
 * Architecture layers:
 * 1. DOM extraction: Injects script to identify interactive elements
 * 2. Visual labeling: Draws index numbers on screenshot for LLM perception
 * 3. Dual reference: LLM can use "[33]:<button>" OR (x,y) coordinates
 *
 * WHY vision-based over pure DOM?
 * - Shadow DOM / iframes / dynamic content break selector-based tools
 * - Visual layout understanding (overlapping elements, modals, z-index)
 * - Human-like interaction (LLM sees what user sees)
 *
 * Design tradeoffs:
 * - Pro: Handles any web UI (even canvas/SVG apps)
 * - Con: Screenshot tokens expensive (requires vision models)
 * - Pro: Resilient to DOM structure changes
 * - Con: Label overlaps require fallback heuristics
 *
 * Performance modes (config.mode):
 * - "fast": No screenshots, DOM-only (cheaper tokens)
 * - default: Single labeled screenshot per action
 * - "expert": Double screenshots (original + labeled) for validation
 *
 * Label drawing modes (config.markImageMode):
 * - "dom": Inject CSS to highlight elements (lightweight)
 * - "draw": Canvas API draws boxes on screenshot (more reliable)
 */
...
* Parallelism:
  ...
  /**
   * Captures screenshot + builds labeled element tree
   *
   * Process flow:
   * 1. Injects build_dom_tree script (marks interactive elements)
   * 2. Executes get_clickable_elements (returns indices + coordinates)
   * 3. Takes screenshot and compresses to reduce token cost
   * 4. Overlays bounding boxes with index labels (if markImageMode="draw")
   * 5. Returns dual outputs: labeled screenshot + pseudoHtml element list
   *
   * Retry logic: 5 attempts with delays for dynamic page stabilization.
   * Double screenshots: Returns original + labeled for expert mode validation.
   *
   * WHY pseudoHtml? Structured element list provides context when labels
   * overlap or are off-screen (e.g., "[33]:<button>Submit</button>").
   */
  ...
  /**
   * Screenshot compression with viewport-aware resizing
   *
   * Strategy: Resize screenshot to match viewport dimensions (removes
   * pixel density overhead from high-DPI screens). Reduces token cost
   * while preserving visual accuracy for element recognition.
   */
  ...
  /**
   * Generates script to retrieve element by index
   *
   * Uses injected window.get_highlight_element() from build-dom-tree script
   * (maintains mapping of index → DOM element across tool calls).
   */
  ...
  /**
   * Determines if tool calls can execute in parallel
   *
   * Blacklist approach: Prevents parallelism for state-changing operations
   * (wait, navigate_to, switch_tab, scroll_mouse_wheel) that may cause
   * race conditions or DOM invalidation.
   *
   * WHY allow parallelism? Form filling benefits from concurrent field
   * updates (e.g., input_text on multiple non-dependent fields).
   */
  ...
  /**
   * Injects updated screenshot + pseudoHtml after each action
   *
   * Strategy: After tool execution (except extract_page_content/get_all_tabs),
   * captures new state and appends to message history. This provides LLM
   * with latest visual feedback loop (sees result of previous action).
   *
   * Optimization: Removes redundant attributes (class, src, href) from
   * older messages to reduce context size (indices more important than
   * full HTML).
   */
  ...
  /**
   * Strips verbose attributes from historical pseudoHtml
   *
   * Context optimization: Older messages (not first 2, not last 3) have
   * class/src/href removed since indices remain stable. Reduces token
   * cost while preserving recent action context.
   */
  ...
  /**
   * Line-by-line attribute removal from pseudoHtml
   *
   * Pattern: "[index]:<element attrs>" → keeps index, removes specified attrs
   * (regex-free approach using indexOf for stability).
   */
...
/**
 * Injected browser script: Types text into element by index
 *
 * Strategy:
 * 1. Resolves element (supports iframe/shadow DOM traversal)
 * 2. Finds input target (input/textarea/contenteditable)
 * 3. Sets value (handles React-like frameworks with descriptor setter)
 * 4. Dispatches input event for change detection
 * 5. Optionally simulates Enter key press
 *
 * WHY descriptor setter? React intercepts value changes, requiring
 * __proto__ access to trigger synthetic events properly.
 */
...
/**
 * Injected browser script: Simulates mouse click on element
 *
 * Event sequence: mousedown → mouseup → click (mimics real user interaction
 * for JavaScript event handlers). Dispatches to shadow DOM roots via
 * element.click() fallback.
 *
 * Supports: left/right/middle clicks, multiple clicks (double/triple)
 */
...
/**
 * Injected browser script: Triggers hover state on element
 *
 * Use case: Reveal dropdown menus, tooltips, or dynamic content that
 * requires mouseenter event (not clickable but interaction-dependent).
 */
...
/**
 * Injected browser script: Extracts <select> dropdown options
 *
 * Returns: Array of {index, text, value} for LLM to choose from.
 * Error handling: Validates element is <SELECT> tag.
 */
...
/**
 * Injected browser script: Selects option in native <select> dropdown
 *
 * Matching strategy: First tries exact text match, falls back to value
 * match (handles options with different display text vs value).
 * Dispatches change event for framework reactivity.
 */
...
/**
 * Injected browser script: Intelligent scrolling with fallback strategies
 *
 * Algorithm:
 * 1. If document scrollHeight > viewport (simple case), scroll window
 * 2. Otherwise, find scrollable container elements (overflow-y: auto/scroll)
 * 3. Rank by z-index (top layer priority) + visible area (size heuristic)
 * 4. Scroll highest-ranked element + max-height element (handles modals)
 *
 * WHY complex ranking? Pages with modals/dialogs require scrolling the
 * overlay container, not background page. Z-index + visibility identify
 * the interaction target.
 *
 * Includes iframe traversal for nested scrollable regions.
 */
